...
const Courses: React.FC = () => {
  ...
  const getCourses = async () => {
    ...
    const coursesData = coursesSnapshot.docs.map((doc) => doc.data() as CourseModel);
    setCourses([...coursesData]); 
    console.log('Updated courses:', coursesData);
  };
...
//     const coursesData = coursesSnapshot.docs.map((doc) => doc.data() as CourseModel);
//     setCourses([...coursesData]);
//     console.log('Updated courses:', coursesData); // Ensures courses data is correctly fetched and set
//   };
...
//   if (courses.length === 0) return null; // Prevent rendering until courses are loaded
