      ...
      <div className="max-w-7xl mx-auto flex flex-wrap">
        ...
        <div className="w-full md:w-1/2 pr-4 overflow-y-scroll scrollbar-cyan " style={{ maxHeight: '400px' }}>
          {reviews.map((review, index) => (
            <div
              key={index}
              className={`transition-transform duration-300 mb-4 m-2 ${hoveredIndex === index ? 'z-[10]' : 'z-0'}`}
              onMouseEnter={() => handleMouseEnter(index)}
              onMouseLeave={handleMouseLeave}
            >
              {/* Color strip */}
              <div className={`bg-${hoveredIndex === index ? 'purple' : 'gray'} h-full w-2 absolute left-0 top-0 bottom-0`}></div>

              <div
                className={`flex items-center border-2 border-black-300 rounded-lg p-2 md:p-6 transition-transform duration-300 ${
                  hoveredIndex === index ? 'transform -translate-x-5 md:-translate-x-6' : ''
                }`}
                style={{ marginLeft: hoveredIndex === index ? '20px' : '0', marginRight: hoveredIndex === index ? '20px' : '0' }}
              >
                <img
                  src={review.image}
